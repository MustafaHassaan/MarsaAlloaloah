...
namespace MarsaAlloaloah
{
    public partial class frmRole : Form
    {
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            {
                Interaction.MsgBox("الرجاء إدخال اسم الصلاحية.", MsgBoxStyle.Information, "الصلاحيات");
                return;
            }
        ...
        private void AddRole()
        {
            try
            {
                SQLConn.sqL = "INSERT INTO Role(Name) VALUES('" + txtRole.Text + "')";
                SQLConn.ConnDB();
        ...
        private void UpdateRole()
        {
            try
            {
                SQLConn.sqL = "UPDATE Role SET Name= '" + txtRole.Text + "' WHERE ID = '" + SelectedRoleID + "'";
                SQLConn.ConnDB();
                SQLConn.cmd = new SqlCommand(SQLConn.sqL, SQLConn.conn);
                SQLConn.cmd.ExecuteNonQuery();
        ...
        private void dgvRoles_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            try
            {
                if (e.RowIndex != -1)
                {
                    SelectedRoleID = Convert.ToInt32(dgvRoles.CurrentRow.Cells["dgvRoleID"].Value);
                    FillControls();
                    btnSave.Text = "تحديث";
        ...
        public void FillControls()
        {
            try
            {
                ...
                txtRole.Text = RoleInfo.Rows[0]["Name"].ToString();
            }
            catch (Exception ex)
            {
                Interaction.MsgBox(ex.ToString());
        ...
        public DataTable RoleView(int RoleID)
        {
            ...
            try
            {
                ...
                SQLConn.sqL = "select * from Role where ID = @id";
                SQLConn.ConnDB();
                SQLConn.cmd = new SqlCommand(SQLConn.sqL, SQLConn.conn);
        ...
        private void DeleteFunction()
        {
            ...
            try
            {
                SQLConn.sqL = "Delete From Role WHERE ID = " + SelectedRoleID + "";
                SQLConn.ConnDB();
                SQLConn.cmd = new SqlCommand(SQLConn.sqL, SQLConn.conn);
                SQLConn.cmd.ExecuteNonQuery();
                Interaction.MsgBox("تم الحذف بنجاح ", MsgBoxStyle.Information, "حذف صلاحية");
                Gridfill();
            }
            catch (Exception ex)
            {
        ...
        private void btnClose_Click(object sender, EventArgs e)
        {
            try
            {
                if ((MessageBox.Show("هل تريد بالتأكيد الإنهاء", "الصلاحيات", MessageBoxButtons.YesNo, MessageBoxIcon.Question)) == DialogResult.Yes)
                {
                    this.Close();
                }
            }
